...
$q_tanggal = mysqli_query($conn, "
    SELECT tanggal_kunjung 
    FROM booking_wisata 
    WHERE id_wisata = '$id'
...
$tanggal_booked = [];
while($t = mysqli_fetch_assoc($q_tanggal)){
    $tanggal_booked[] = $t['tanggal_kunjung'];
}
    ...
    <div class="row g-4">
                ...
                <div class="carousel-inner">
                    <?php foreach($list_foto as $index => $foto): ?>
                        <div class="carousel-item <?= $index === 0 ? 'active' : '' ?>">
                            <img src="<?= $foto['src'] ?>" class="d-block w-100" style="height: 600px; object-fit: cover;" alt="<?= $foto['alt'] ?>">
                        </div>
                    <?php endforeach; ?>
                </div>
        ...
        <div class="col-md-6">
           ...
           <div class="d-flex flex-wrap gap-2 mt-2">
            <?php foreach($tanggal_booked as $tgl): ?>
                <span class="badge bg-danger-subtle text-danger border border-danger px-3 py-2 rounded-3">
        ...
        <small class="text-muted d-block mt-2">
            Tanggal di atas tidak bisa dipilih saat booking.
        </small>
    </div>
            <?php endif; ?>
                        ...
                        <div class="d-grid gap-2 d-md-block">
                            <?php if(count($tanggal_booked) >= 7): ?>
                <button class="btn btn-secondary btn-lg w-100" disabled>
                    <i class="bi bi-calendar-x me-2"></i> Tanggal Penuh
                </button>
            <?php else: ?>
...
navigator.geolocation.getCurrentPosition(
    function(pos) {
        ...
        // Google Maps Directions API untuk jarak jalan sebenarnya
